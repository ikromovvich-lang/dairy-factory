...
router.get('/', authenticate, async (req, res) => {
  ...
  if (search) { query += ` AND (f.name ILIKE $${params.length+1} OR f.phone ILIKE $${params.length+1})`; params.push(`%${search}%`); }
  if (is_active !== undefined) { query += ` AND f.is_active = $${params.length+1}`; params.push(is_active === 'true'); }
...
router.delete('/:id', authenticate, authorize('admin'), async (req, res) => {
  await pool.query('UPDATE farmers SET is_active=false WHERE id=$1 AND factory_id=$2', [req.params.id, req.user.factory_id]);
