...
// Get inventory detail with batches
router.get('/batches', authenticate, async (req, res) => {
  ...
  if (product_type) { query += ` AND i.product_type = $${params.length+1}`; params.push(product_type); }
...
router.get('/movements', authenticate, async (req, res) => {
  const result = await pool.query(`
    SELECT 
      'production' as type, pb.production_date as date, pb.product_type,
      pb.quantity_produced as quantity, pb.unit, pb.batch_number
    FROM production_batches pb WHERE pb.factory_id = $1
    UNION ALL
    SELECT 
      'sale' as type, s.sale_date as date, si.product_type,
