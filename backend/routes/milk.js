...
router.post('/', authenticate, async (req, res) => {
  ...
  const farmerResult = await pool.query(
    'SELECT * FROM farmers WHERE id = $1 AND factory_id = $2',
    [farmer_id, req.user.factory_id]
  );
...
router.get('/report/daily', authenticate, async (req, res) => {
  ...
  const summary = await pool.query(`
    SELECT COUNT(*) as delivery_count, SUM(liters) as total_liters,
      AVG(fat_percent) as avg_fat, AVG(protein_percent) as avg_protein,
    ...
    FROM milk_deliveries WHERE factory_id = $1 AND DATE(delivery_date) = $2
